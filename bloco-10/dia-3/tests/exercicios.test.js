...
describe('#Exercício 4 e 5', () => {
  ...
  test("#5 Utilizando as mesmas funções do exercício anterior, repita a implementação para a primeira função. Após repetir a implementação, restaure a implementação original e crie os testes necessários para validar.", () => {
    stringFnc.textToUpperCase.mockRestore();
    stringFnc.firstLength.mockRestore();
    stringFnc.concatStrings.mockRestore();
    
    expect(stringFnc.textToUpperCase('TaLes')).toBe('TALES');
    expect(stringFnc.firstLength('TaLes')).toBe('T');
